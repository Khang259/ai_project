...
async def get_task_details_by_total_duration_gt(
    ...
    """
    Lấy dữ liệu từ collection task_details có total_duration lớn hơn threshold.
    Document có các trường: order_id, Get_shelf, Transition, Shelf_lifting,
    Shelf_transport, Shelf_release, total_duration, (hoặc Cancel), ...
    """
    task_details_collection = get_collection("task_details")

    query = {"total_duration": {"$gt": threshold}}
    ...
    # Chuyển _id sang string nếu có (để JSON serializable)
