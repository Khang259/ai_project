...
async def login(user_in: UserLogin, request: Request):
    try:
        # Log raw body (mask password) for debugging 422/validation issues
        raw = await request.body()
        body_text = raw.decode("utf-8", errors="ignore") if isinstance(raw, (bytes, bytearray)) else str(raw)
        safe_log = body_text.replace("\"password\":\"", "\"password\":\"***")
        logger.info(f"/auth/login payload: {safe_log}")
    except Exception:
        pass
    ...
    token = create_user_token(user)
    logger.info(f"Token created for username='{token}'")
    user_info = await get_current_user_info(str(user["_id"]))
    return {"access_token": token, "token_type": "bearer", "user": user_info}
