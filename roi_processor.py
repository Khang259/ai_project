...
class ROIProcessor:
    def __init__(self, db_path: str = "queues.db", show_video: bool = True, enable: int = 1):
        """
        ...
        self.video_display_manager = VideoDisplayManager(show_video, enable_dict=self.enable_dict)
        ...
        self.pairing_config_path: str = "slot_pairing_config.json"
    ...
    def toggle_enable(self, enable: int) -> dict:  # ← FIX: int parameter
        """
        Bật tắt ROI Processor
        
        Args:
            enable: 0 (disabled) hoặc 1 (enabled)
        """
        if enable not in [0, 1]:
            return {"error": "Invalid enable value. Must be 0 or 1"}
        
        self.enable_dict['value'] = enable
        
        # ✅ FIX: Propagate đến VideoDisplayManager
        if hasattr(self, 'video_display_manager'):
            print(f"[ROI_PROCESSOR] Propagated enable={enable} to VideoDisplayManager")
            return {"success": True, "message": f"ROI Processor enabled", "enable": enable}
        
        return {"success": False, "message": "VideoDisplayManager not found"}
