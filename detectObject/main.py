...
class CameraOrchestrator:
    ...
    def __init__(self, camera_urls, num_processes=4, max_retry_attempts=5, use_ai=True, model_path="yolov8n.pt", target_fps=1.0):
        ...
        self.processes = []
        self.ai_enabled = Value('i', 1 if use_ai else 0)  # Shared memory
    ...
    def start(self):
        ...
        try:
            ai_process = Process(
                ...
                args=(self.shared_dict, self.result_dict, self.model_path, self.target_fps, self.ai_enabled)
            )
            self.processes.append(ai_process)
            ai_process.start()
            print("Đã khởi động AI inference")
        except Exception as e:
            print(f"Lỗi khởi động AI inference: {e}")
        
        print(f"Đã khởi động {len(self.processes)} process camera")
        print(f"AI Detection: {'BẬT' if self.use_ai else 'TẮT'}")
    ...
    def toggle_ai(self, enable: bool):
        self.ai_enabled.value = 1 if enable else 0
        return {"success":"Switching model to " + ("enabled" if enable else "disabled")}
