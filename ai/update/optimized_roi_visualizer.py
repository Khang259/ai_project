...
class VideoDisplayManager:
    ...
    def _update_local_dict_from_processor(self):
        ...
        Đảm bảo single source of truth và không có race condition
        """
        try:
            # Update ROI cache
            ...
            # GIẢI PHÁP 3: Sync blocking state từ roi_processor.blocked_slots (Single Source of Truth)
            # Thay vì dùng self.blocked_rois từ subscribe topics (có thể bị delay),
            # ta sync trực tiếp từ roi_processor để đảm bảo đồng bộ tức thì
            if self._blocked_slots_ref:
                # Clear old blocked_rois và rebuild từ source of truth
                self.blocked_rois.clear()
                
                for camera_id, blocked_slots_dict in self._blocked_slots_ref.items():
                    if blocked_slots_dict:  # Nếu camera có slots bị block
                        self.blocked_rois[camera_id] = {}
                        for slot_number, expire_time in blocked_slots_dict.items():
                            # Copy blocking info vào visualizer
                            self.blocked_rois[camera_id][slot_number] = {
                                "blocked_at": time.time(),
            ...
            # Update blocked ROIs cache vào local_dict để display threads sử dụng
            for camera_id, blocked_slots in self.blocked_rois.items():
                self.local_dict[f'{camera_id}_blocked'] = blocked_slots
            
            # Clear blocked state cho cameras không còn trong blocked_slots_ref
    ...
    def display_video(self, roi_cache: Dict, latest_roi_detections: Dict,
                      ...
                      blocked_slots: Optional[Dict] = None):
        """
        ...
        self._blocked_slots_ref = blocked_slots  # GIẢI PHÁP 3: Reference đến source of truth
        ...
        # GIẢI PHÁP 3: KHÔNG CẦN subscribe blocking topics nữa
        # Vì đã sync trực tiếp từ roi_processor.blocked_slots trong _update_local_dict_from_processor()
        # Điều này đảm bảo:
        # - Single source of truth (roi_processor.blocked_slots)
        # - Không có race condition
        # - Đồng bộ tức thì khi update detection data
        
        # # Khởi động blocking subscription thread (dual + manual API) - KHÔNG CẦN NỮA
        # if self.queue:
