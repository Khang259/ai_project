...
RETRY_DELAY_SECONDS = 2  # Delay between POST retries
...
# def send_post(payload: Dict[str, Any], logger: logging.Logger) -> bool:
#     # Headers giống hệt Postman (copy từ Postman > Code > Python requests)
#     headers = {
#         "User-Agent": "PostmanRuntime/7.37.3",       # cập nhật đúng phiên bản Postman bạn đang dùng
#         "Accept": "*/*",
#         "Accept-Encoding": "gzip, deflate, br",
#         "Connection": "keep-alive",
...
#         task_path = task_order_detail[0].get('taskPath', 'N/A')
#     elif len(task_order_detail) == 2:
#         task_path = f"{task_order_detail[0].get('taskPath','N/A')} | {task_order_detail[1].get('taskPath','N/A')}"
#     else:
#         task_path = 'N/A'
...
#     try:
#         # CRITICAL FIX 1 + 2: dùng json= + compact JSON
#         resp = requests.post(
#             API_URL,
#             json=payload,                                    # đây là cách đúng nhất
#             headers=headers,
#             timeout=15
#         )
        
#         # CRITICAL FIX 3: đảm bảo đọc hết response
#         try:
#             body = resp.json()
#         except:
#             body = {"raw": resp.text}
        
#         # Bắt buộc đọc .content để server biết client đã nhận hết
#         _ = resp.content  # dòng này quan trọng hơn bạn nghĩ đấy!
        
#         print(f"=== POST RESPONSE ===")
#         print(f"Status Code: {resp.status_code}")
#         print(f"Response Body: {json.dumps(body, ensure_ascii=False, indent=2)}")
        
#         logger.info(f"POST_RESPONSE_RECEIVED: orderId={order_id}, status_code={resp.status_code}, body={json.dumps(body, ensure_ascii=False)}")

#         # Server của bạn dùng code=1000 là thành công
#         if resp.status_code < 400 and body.get("code") == 1000:
#             print(f"[SUCCESS] POST thành công + code=1000 | OrderID: {order_id}")
#             logger.info(f"POST_REQUEST_SUCCESS: orderId={order_id}, response_code=1000")
#             return True
#         else:
#             print(f"[FAIL] HTTP {resp.status_code} hoặc code != 1000 | Got: {body.get('code')}")
#             logger.warning(f"POST_REQUEST_FAILED: orderId={order_id}, http_code={resp.status_code}, api_code={body.get('code')}")
#             return False
            
#     except requests.exceptions.Timeout:
#         print(f"[ERROR] Timeout sau 15s")
#         logger.error(f"POST_REQUEST_TIMEOUT: orderId={order_id}")
#         return False
#     except Exception as e:
#         print(f"[ERROR] Lỗi không mong muốn: {e}")
#         logger.error(f"POST_REQUEST_EXCEPTION: orderId={order_id}, error={str(e)}")
#         return False

def send_post(payload: Dict[str, Any], logger: logging.Logger) -> bool:
   
    # Chuyển payload thành JSON KHÔNG formatting thêm, giống Postman
    ...
    try:
        resp = requests.post(
            API_URL,
            data=body_raw,          # dùng data= để gửi RAW body
            headers=headers,
            timeout=None
        )
