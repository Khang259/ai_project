        ...
        mapData.nodeArr.forEach((node) => {
          try {
            ...
            if (typeof node.name === 'string' && /^Camera\s*_?\s*\d+$/i.test(node.name.trim())) {
              ...
              // Extract camera ID từ tên (dùng cho online state và fallback map)
              ...
              // Map theo camera_name (sau khi chuẩn hóa), fallback theo camera_id
              const cameraFromDB =
                camerasData.find((c) => normalizeName(c.camera_name) === normalizedNodeName) ||
                camerasData.find((c) => c.camera_id === cameraIndex);
              ...
              ">
                ...
                <div style="font-family: monospace; font-size: 11px;">${cameraPath}</div>
              </div>`, {
                permanent: false,
                direction: 'top',
                offset: [0, -20],
